...
class Node:
    ...
    def expand(self, problem, visited=None):
...
def depth_limited_search(problem, limit=50):
    "[Fig. 3.12]"
    exploredNodes = 0
    def recursive_dls(node, problem, limit, exploredNodes):
        cutoff_occurred = False
        exploredNodes += 1
        if problem.goal_test(node.state):
            return node,exploredNodes
        elif node.depth == limit:
            return 'cutoff'
        else:
            for successor in node.expand(problem):
                result = recursive_dls(successor, problem, limit, exploredNodes)
                if result == 'cutoff':
    ...
    return recursive_dls(Node(problem.initial), problem, limit, exploredNodes)
